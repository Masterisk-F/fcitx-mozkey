...
## 注意事項：「選択文字列の再変換」が文字列を選択していなくても発動する

文字列の末尾で Space キーを押した際に、テキストを選択していないにもかかわらず「選択文字列の再変換」が発動することがあります。

これは本リポジトリのバグではなく、上流の [fcitx/mozc](https://github.com/fcitx/mozc) の仕様です。クリップボードのテキストとカーソル周辺のテキストが一致した場合、その文字列が選択状態であると見なされます。

現在のところ、この挙動だけを回避する方法はありません。「選択文字列の再変換」機能を完全に無効化するか、この挙動を受け入れるかを選択する必要があります。

「選択文字列の再変換」を完全に無効化したい場合は以下の手順を実行してください。
1. Mozc のプロパティ（設定ツール）を開き、「一般」タブの「キー設定の選択」の横にある「編集...」をクリックします。
2. キー設定エディタの一覧から、
   - **モード:** `入力文字なし`
   - **入力キー:** `Space`
   - **コマンド:** `選択文字列を再変換（未選択時は空白）`
   を探し、コマンドを「空白を入力」に変更してください。

（以下は上流 Mozkey の README です）
---
